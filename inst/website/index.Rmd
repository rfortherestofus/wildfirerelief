...
```{r}
map_covid <- function(leaflet_layers, observation_date) {
  
  pretty_date <- stamp_date("March 1")(ymd(observation_date))
  
  group_name = str_glue("COVID Cases {pretty_date}")
...
This shows hospitals in Oregon and Siskiyou.
...
This map shows all of the above maps in a single map, with the ability to filter out certain layers.
...
map_basemap() %>% 
  ...
  map_hospitals() %>% 
  addLayersControl(
    overlayGroups = c("Wildfires", "Hospitals", "Communities"),
    baseGroups = c("Don't Show COVID Cases", 
                   "COVID Cases September 1",
                   "COVID Cases October 1",
                   "COVID Cases November 1"),
